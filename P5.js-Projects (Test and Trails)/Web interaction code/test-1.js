...
let isAnimationRunning = false; // Set initial animation state to false
...
function toggleAnimation() {
  ...
  if (isAnimationRunning) {
    // Start audio processing only if animation is running
    mic = new p5.AudioIn();
    mic.start();
    fft = new p5.FFT();
    fft.setInput(mic);
    ...
    // Stop audio processing if animation is paused
    mic.stop();
    noLoop();
  }
}
...
class HexRing {
  constructor(x, y, diameter) {
    ...
    this.angle = 0;
    this.rotationSpeed = random(-0.02, 0.02)*2;
...
class Particle {
  ...
  update(speed) {
    this.x += this.velocity.x * speed*10;
    this.y += this.velocity.y * speed*10;
